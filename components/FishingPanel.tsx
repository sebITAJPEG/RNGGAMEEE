                ...
                {lastBatch.length > 0 ? (
                    <div key={totalFished} className="animate-fade-in-up w-full flex flex-col gap-2 items-center">
                        {lastBatch.map((fish, idx) => (
                            <div key={idx} className="flex items-center gap-3 bg-black/40 p-2 rounded border border-cyan-900 w-full max-w-[240px]" style={{ animationDelay: `${idx * 0.05}s` }}>
                                <div className={`text-[9px] font-mono uppercase px-1.5 py-0.5 rounded bg-neutral-900 border border-neutral-800 min-w-[40px] text-center ${fish.color.replace('text-', 'text-')}`}>
                                    {fish.tierName}
                                </div>
            ...
            <button
                ...
                className={`
                    ...
                    ${isAutoFishing 
                        ...
                        : 'bg-transparent border-neutral-800 text-neutral-600 hover:border-cyan-900 hover:text-cyan-500'
                    }
                `}
            >
