...
export const Shop: React.FC<Props> = ({
    ...
    const UpgradeCard = ({ title, desc, current, next, cost, isMaxed, onBuy, colorClass, borderColor }: any) => (
        <div className={`p-4 bg-surface/50 rounded border ${borderColor} flex flex-col justify-between h-full`}>
            <div>
                <h4 className={`text-sm font-bold font-mono mb-1 ${colorClass}`}>{title}</h4>
                <p className="text-xs text-text-dim font-mono mb-3 min-h-[3rem]">{desc}</p>
            ...
            {isMaxed ? (
                ...
                <button
                    ...
                    ${balance >= cost
                            ? `bg-surface-highlight hover:bg-secondary ${colorClass} ${borderColor}`
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-background/80 backdrop-blur-md p-4">
            <div className="w-full max-w-5xl bg-surface border border-border rounded-lg shadow-2xl flex flex-col max-h-[85vh]">
                <div className="flex justify-between items-center p-6 border-b border-surface-highlight">
                    <h2 className="text-xl font-bold text-text tracking-wider font-mono">UPGRADE STATION</h2>
                    <div className="flex items-center gap-4">
                        <div className="text-right">
                            <span className="block text-xs text-text-dim font-mono">BALANCE</span>
                            <span className="block text-lg text-yellow-500 font-bold font-mono">{balance.toLocaleString()} PTS</span>
                        </div>
                        <button onClick={onClose} className="text-text-dim hover:text-text font-mono ml-4">[X]</button>
                    </div>
                </div>
                ...
                <div className="p-6 overflow-y-auto space-y-8">
                    ...
                    {/* MAIN SECTION */}
                    <div>
                        <h3 className="text-sm font-bold text-text font-mono mb-4 border-b border-surface-highlight pb-2">SYSTEM UPGRADES</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="p-4 bg-surface/50 rounded border border-purple-900 flex flex-col justify-between h-full relative overflow-hidden">
                                <div>
                                    <h4 className="text-sm font-bold font-mono mb-1 text-purple-400">BURST GPU</h4>
                                    <p className="text-xs text-text-dim font-mono mb-3">Unlock 50x Burst Roll button.</p>
                                </div>
                                {stats.hasBurst ? (
                    ...
                    {onBuyFishingSpeed && (
                        <div>
                            <h3 className="text-sm font-bold text-text font-mono mb-4 border-b border-surface-highlight pb-2">FISHING MODULES</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <UpgradeCard
                    ...
                    {onBuyHarvestingSpeed && (
                        <div>
                            <h3 className="text-sm font-bold text-text font-mono mb-4 border-b border-surface-highlight pb-2">HARVESTING MODULES</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <UpgradeCard
